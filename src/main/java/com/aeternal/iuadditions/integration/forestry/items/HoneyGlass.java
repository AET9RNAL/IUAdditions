...
public class HoneyGlass extends IUAItemSubTypes<HoneyGlass.HoneyGlassTypes> implements IModelRegister {
    ...
    public enum HoneyGlassTypes implements ISubEnum {
        shimmering(0),
        radioactive(1),
        venomous(2),
        certus(3),
        ...
        HoneyGlassTypes(final int ID) {
            this.name = this.name().toLowerCase(Locale.US);
            this.ID = ID;
        }

        public static HoneyGlass.HoneyGlassTypes getFromID(final int ID) {
